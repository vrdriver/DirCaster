...
/*
   * DirCaster 1.0, released 2020-Dec-01. http://www.DirCaster.org
   *
   * ChangeLog - See "changelog for DirCaster.txt"
   *
   * Do not edit anything in this file, unless you are a programmer
   * All individual site configuration is done in the file: config_inc.php
   *
   * Main Code
  */
  ...
  */
  ...
  // Alter description if file type request is bad
  if ( !$goodParam ){
  ...
  function stripJunk($text) {
    ...
    for ($c = 0; $c < strlen($text); $c++) {
        if (ord($text[$c]) >= 32 && ord($text[$c]) <= 122) {
            $outText .= $text[$c];
        }
    }
    return $outText;
...
if (empty($aItems['artist'])) {
  $artist = $mp3file->artist ?? ''; // Use empty string if null
...
// nonremote album
if (empty($aItems['album'])) {
  $album = $mp3file->album ?? '';
  $album = htmlentities($album, ENT_QUOTES, 'UTF-8');
} else {
  $album = $aItems['album'];
}

// nonremote composer
if (empty($aItems['composer'])) {
  $composer = $mp3file->composer ?? '';
  $composer = htmlentities($composer, ENT_QUOTES, 'UTF-8');
} else {
  $composer = $aItems['composer'];
}

// nonremote genre
if (empty($aItems['genre'])) {
  $genre = $mp3file->genre ?? '';
...
// nonremote track
$track = !empty($aItems['track']) ? $aItems['track'] : '';

// nonremote copyright
if (empty($aItems['copyright'])) {
  $copyright = $mp3file->copyright ?? '';
  $copyright = htmlentities($copyright, ENT_QUOTES, 'UTF-8');
} else {
  $copyright = $aItems['copyright'];
}
...
// Ensure $duration is properly formatted before splitting
if (!is_string($duration) || empty($duration)) {
  $duration = '00:00:00'; // Default to zero duration if null or invalid
}
